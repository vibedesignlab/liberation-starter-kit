...
def asset_figure(asset: dict) -> str:
    asset_id = text(asset.get("asset_id"))
    file_path = text(asset.get("file_path"))
    return f'<figure class="asset"><img src="../{html.escape(file_path)}" alt="{html.escape(asset_id)} {html.escape(text(asset.get("product_name")))}"><figcaption><span>{html.escape(asset_id)} · {html.escape(text(asset.get("product_name")))}</span><span>{html.escape(file_path)}</span></figcaption></figure>'
...
"""
    ...
    for index, product in enumerate(products, 5):
        ...
        figure = asset_figure(asset) if isinstance(asset, dict) else ""

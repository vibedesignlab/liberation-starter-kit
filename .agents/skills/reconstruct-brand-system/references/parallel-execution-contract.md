...
Generate the representative-product hero and brand-mood anchor concurrently by default. Serialize them only when the mood image must use the finished hero image as a direct geometry reference. While images run, prepare the canonical model, prompt records, registry skeleton, and report shell.

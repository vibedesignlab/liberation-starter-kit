...
def asset_block(item: dict) -> str:
    ...
    prompt_path = html.escape(text(item.get("prompt_path")))
    status = html.escape(text(item.get("status")) or "pending")
    if not file_path or item.get("status") != "registered":
        return (
            f'<article class="asset pending-asset" data-asset-status="{status}">'
            f'<span class="asset-status">{status}</span><h3>{asset_id} · {role}</h3>'
            f'<p>{job}</p><p class="prompt-path">{prompt_path}</p></article>'
        )
    return (
        f'<figure class="asset"><img src="../{html.escape(file_path)}" alt="{asset_id} {role}">'
        f'<figcaption><span>{asset_id} · {role}</span><span>{html.escape(file_path)}</span>'
        f'<span>{prompt_path}</span></figcaption>'
...
def main() -> int:
    ...
    for item in assets:
        if isinstance(item, dict):
            assets_by_role.setdefault(text(item.get("role")), []).append(item)
    ...
    for number, (key, title) in enumerate(SECTION_ORDER, 1):
        ...
        if key == "brand_mood_and_brand_imagery":
            extra += "".join(asset_block(item) for item in assets_by_role.get("brand_mood", []))
        if key == "product_visual_traits_and_product_imagery":
            extra += "".join(asset_block(item) for item in assets_by_role.get("representative_product_hero", []))
        section_html.append(
...
<style>:root{{--paper:#f3f1ea;--ink:#171918;--muted:#69706b;--line:#c9ccc5;--brand:{bg};--brand-ink:{fg};--accent:{accent};--pad:clamp(24px,5vw,72px)}}*{{box-sizing:border-box}}body{{margin:0;background:var(--paper);color:var(--ink);font-family:Arial,"Helvetica Neue",sans-serif;line-height:1.5}}main{{display:grid;grid-template-columns:minmax(104px,13vw) minmax(0,1fr)}}aside{{position:sticky;top:0;height:100vh;padding:24px 18px;border-right:1px solid var(--line);display:flex;flex-direction:column;justify-content:space-between;font-size:10px;letter-spacing:.09em;text-transform:uppercase}}.section{{padding:var(--pad);border-bottom:1px solid var(--line);display:grid;grid-template-columns:minmax(118px,190px) minmax(0,1fr);gap:clamp(24px,6vw,96px)}}.hero{{min-height:78vh;align-items:end;background:var(--brand);color:var(--brand-ink)}}.meta{{font-size:10px;letter-spacing:.09em;text-transform:uppercase}}h1,h2{{margin:0 0 24px;line-height:1;letter-spacing:-.045em}}h1{{font-size:clamp(48px,7vw,100px)}}h2{{font-size:clamp(30px,4vw,58px)}}h3{{font-size:18px;margin:22px 0 8px}}p,li{{font-size:16px;max-width:820px;overflow-wrap:anywhere}}.lede{{font-size:clamp(18px,2vw,27px);max-width:760px}}.record{{border-top:1px solid var(--line);padding:14px 0}}.record>.record{{margin-left:18px}}.cards,.product-grid{{display:grid;grid-template-columns:repeat(2,minmax(0,1fr));gap:1px;background:var(--line);border:1px solid var(--line)}}.card,.product-card{{background:var(--paper);padding:20px}}.product-grid{{margin-top:28px}}.product-card span{{font-size:10px;text-transform:uppercase;letter-spacing:.08em}}.asset{{margin:30px 0 0}}.asset img{{width:auto;max-width:100%;height:auto;display:block;background:#ddd}}.pending-asset{{border:1px solid var(--line);padding:20px;background:rgba(255,255,255,.35)}}.asset-status{{font-size:11px;letter-spacing:.08em;text-transform:uppercase;color:var(--muted)}}.prompt-path{{font-family:ui-monospace,SFMono-Regular,Menlo,monospace;font-size:13px;overflow-wrap:anywhere}}figcaption{{display:flex;flex-wrap:wrap;justify-content:space-between;gap:12px;border-top:1px solid var(--line);padding:9px 0;font-size:10px;overflow-wrap:anywhere}}.caption{{font-size:13px;color:var(--muted)}}.checkpoint{{background:var(--ink);color:var(--paper)}}.checkpoint strong{{color:var(--accent)}}@media(max-width:850px){{main{{display:block}}aside{{position:relative;height:auto;flex-direction:row;border-right:0;border-bottom:1px solid var(--line)}}.section{{grid-template-columns:1fr}}}}@media(max-width:560px){{.cards,.product-grid{{grid-template-columns:1fr}}figcaption{{display:block}}}}</style></head>

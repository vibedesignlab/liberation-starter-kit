...
when_to_use: Invoke for source-brand anatomy research, reference-brand reconstruction, or Stage 1 of the project-local brand pipeline. Do not invoke for target-brand invention, transfer planning, landing materials, or prompt-only commercial photography work.
compatibility: Requires Python 3 and the project's pnpm scripts. Evidence retrieval may use network-capable search, direct HTTP, feeds, APIs, and official downloads, but browser automation requires an explicit user request.
---
...
- Before evidence work, read [resources/evidence-and-layer-model.md](resources/evidence-and-layer-model.md).
- Before anatomy and grammar, read [resources/source-anatomy-schema.md](resources/source-anatomy-schema.md).
- Before the global system, read [resources/global-brand-system-framework.md](resources/global-brand-system-framework.md).
- Before delivery, read [resources/report-language-style.md](resources/report-language-style.md).
- Before exporting the handoff JSON, read [resources/brand-model-json-contract.md](resources/brand-model-json-contract.md).
- Before registering the reader document, read the Claude router's [Storybook report contract](../reconstruct-brand-system/resources/storybook-report-contract.md).

## Initialize

```bash
CLAUDE_RESEARCH_SKILL_DIR=".claude/skills/research-brand-anatomy"
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/init_analysis.py" <analysis-directory>
```
...
For standalone use, Claude subagents remain optional. In a routed full pipeline whose state declares `parallel_single_brand`, follow the Claude router's [parallel execution contract](../reconstruct-brand-system/resources/parallel-execution-contract.md): after scope lock, use available worker slots for the three non-overlapping research lanes and keep the main Claude coordinator as the only canonical writer. If slots are unavailable, record a serial fallback reason and continue. Do not add a subjective audit agent.
...
```bash
CLAUDE_RESEARCH_SKILL_DIR=".claude/skills/research-brand-anatomy"
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/export_brand_model.py" <analysis-directory>
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/render_report.py" <analysis-directory>
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/package_digests.py" <analysis-directory>
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/validate_analysis.py" <analysis-directory> all
python3 "$CLAUDE_RESEARCH_SKILL_DIR/scripts/validate_report_language.py" <analysis-directory>/outputs/source-brand-analysis.html
...
If `pipeline_state_path` is registered and the user approves, do not stop after editing the review JSON manually. Run the Claude router's `advance_pipeline.py` with the user's decision and feedback. On `CHAIN_ACTION=CALL_SKILL`, immediately invoke `/build-brand-from-anatomy` using the printed source and destination package paths. On a revision request, remain in this skill.
